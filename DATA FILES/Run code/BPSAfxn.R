...
BPSA.fxn = function(input.data,savefile,foldername){
  ...
  for(m in 1:M){
    ...
    for(k in 1:K){
      ...
      nn.matching = PS.analysis(BPSA.est.PS[[m]][,k],treatment,outcome,covardataset,"nn.matching",sandwich.est=FALSE)
      ...
      caliper.matching = PS.analysis(BPSA.est.PS[[m]][,k],treatment,outcome,covardataset,"caliper.matching",bpsa=2,matching.ratio=1)
      ...
      caliper.matching.5 = PS.analysis(BPSA.est.PS[[m]][,k],treatment,outcome,covardataset,"caliper.matching",bpsa=2,matching.ratio=5)
      ...
      ###estimation via DR
      dr = PS.analysis(BPSA.est.PS[[m]][,k],treatment,outcome,covardataset,"dr")
      ...
      ###estimation via IPW
      ipw = PS.analysis(BPSA.est.PS[[m]][,k],treatment,outcome,covardataset,"ipw",sandwich.est=FALSE)
      ...
      strat = PS.analysis(BPSA.est.PS[[m]][,k],treatment,outcome,covardataset,method="stratification",bpsa=2)
   ...
   # BPSA.sim.results$MCMC.SE[startit:endit] = c(sd(nn.matchingMCMC,na.rm=TRUE),
   ...
   #                                       sd(stratMCMC,na.rm=TRUE))
    ...
    BPSA.sim.results$ATE[startit:endit] = c(mean(nn.matchingATE,na.rm=TRUE),
    ...
    BPSA.sim.results$avg.est.SE[startit:endit] = c(mean(nn.matchingSE,na.rm=TRUE),
    ...
    BPSA.sim.results$rubinse[startit:endit] = c(rubinse(nn.matchingATE,nn.matchingSE),
    ...
    BPSA.sim.results$percused[startit:endit] =c(mean(nn.matchingPU,na.rm=TRUE),
 ...
 #   BPSA.sim.results$num.na[startit:endit] =c(mean(is.na(nn.matchingATE)),
  #                                        mean(is.na(caliper.matchingATE)),
   #                                       mean(is.na(drATE)),
   #                                       mean(is.na(ipwATE)),
   #                                       mean(is.na(stratATE)))
    
    BPSA.sim.results$marginal.SD.nu[startit:endit] = c(mean(apply(nn.matchingNU,1,sd,na.rm=TRUE),na.rm=TRUE),
    ...
    BPSA.sim.results$marginal.SD.nu.treated[startit:endit] = c(mean(apply(nn.matchingNU[treatment==1,],1,sd,na.rm=TRUE),na.rm=TRUE),
    ...
    BPSA.sim.results$marginal.SD.nu.control[startit:endit] =  c(mean(apply(nn.matchingNU[treatment==0,],1,sd,na.rm=TRUE),na.rm=TRUE),
    ...
    BPSA.sim.results$ncontrol[startit:endit] = c(mean(nn.matchingNM[,1],na.rm=TRUE),
                                                 mean(caliper.matchingNM[,1],na.rm=TRUE),mean(caliper.matching.5NM[,1],na.rm=TRUE),
                                                 NA,NA,NA)
    
    BPSA.sim.results$ntreat[startit:endit] = c(mean(nn.matchingNM[,2],na.rm=TRUE),
                                               mean(caliper.matchingNM[,2],na.rm=TRUE),
                                               mean(caliper.matching.5NM[,2],na.rm=TRUE),
                                               NA,NA,NA)
    
    BPSA.sim.results$cov.balance[startit:endit] =c(mean(nn.matchingCB,na.rm=TRUE),
